...
| exact match | 0.556 |
| within ±1 | 0.877 |
| mean(LLM − bench) | +0.531 |
| MAE | 0.580 |
| Pearson r | 0.521 |
| Spearman ρ | 0.542 |

## Per-family presence agreement (Cohen κ)

| family | Cohen κ | LLM-positive n | bench-positive n | n |
|---|---:|---:|---:|---:|
...
The LLM-consensus $d_c$ correlates with the benchmark-author conservation-family count at Spearman ρ = 0.54 (within±1 = 88%). Because $d_c^{\rm bench}$ counts family *presence* while the LLM protocol counts scalar *components*, the LLM tends to score slightly higher on multi-component problems (mean LLM − bench = +0.53). The positive, ordered relationship against an independent, human-authored benchmark annotation supports the construct validity of $d_c$ without requiring us to collect new human labels. A full human-physicist gold pass on the blinded 96-item packet remains the planned final gate.
